...
window.parseFields = function (lines, options = {}) {
  const result = {
    ...
    serial: '',
    category: '',
    brandSource: '', // 'logo' | 'dictionary' | 'fallback'
  };
  ...
  const strongLogo = logos.find((l) => l.score >= 0.5);
  if (strongLogo) {
    // 로고 이름을 사전에서 정규화 시도
    const normalized = matchBrand(strongLogo.description) || strongLogo.description;
